@extends('website.layouts.app')

@section('title', 'Natural Samridhi Yojana')

@section('content')
<div class="abtPage proInnerBanner">
    <div class="banner-sec" style="background-image: url('{{ asset('website/images/rdfc-banner.jpg') }}');">
        <div class="overlay"></div>
        <h3 data-aos="fade-up"
            data-aos-duration="500">Together, We Grow Stronger.
        </h3>
    </div>
</div>
<div class="section  pro_inner_wrapper" style="padding-top: 50px;">

    <div class="projectInnerPage">

        <div class="container rcfc_sec nsy">

            <h5>NATURAL SAMRIDHI YOJANA (NSY)</h5>
            <p>Natural Samridhi Yojana is a savings-based financial scheme designed to promote disciplined investment habits and long-term financial growth among individuals and families.</p>
            <div class="row dv-center">
                <div class="col-md-6 ">
                    <div class="rec_table " style="overflow-x:auto;">
                        <div class="table-responsive">
                        <table>
                            <tr>
                                <th colspan="4" class="bg_green_head">KISSAN SAMRIDHI YOJANA (A)</th>
                            </tr>

                            <tr>
                                <th class="subhead">Year</th>
                                <th class="subhead">Contribution
                                    Amt. YEARLY
                                    (25000*12)</th>
                                <th class="subhead">Pay-out After
                                    6th Year
                                    (25000*12)</th>
                                <th class="subhead">Surrender Benefit</th>

                            </tr>

                            <tr>
                                <td>1</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>2</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>3</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>4</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>5</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>6</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>7</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>8</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>10</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>12</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>14</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>16</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>18</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>20</td>
                                <td>-</td>
                                <td>3,00,000</td>
                                <td>25,00,000</td>
                            </tr>



                        </table>
                        </div>
                    </div>
                    <p class="condition pb-30">*SCHEME ONLY FOR MEMBERS</p>
                </div>
                <div class="col-md-6 ">
                    <div class="rec_table " style="overflow-x:auto; margin-top: -50px;">
                        <table>
                            <tr>
                                <th colspan="4" class="bg_green_head">KISSAN SAMRIDHI YOJANA (B)</th>
                            </tr>

                            <tr>
                                <th class="subhead">Year</th>
                                <th class="subhead">Contribution
                                    Amt. YEARLY
                                    (25000*12)</th>
                                <th class="subhead">Pay-out After
                                    7th Year
                                    (32500*12)</th>
                                <th class="subhead">Surrender Benefit</th>

                            </tr>

                            <tr>
                                <td>1</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>2</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>3</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>4</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>5</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>6</td>
                                <td>3,00,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>7</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>8</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>10</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>12</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>14</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>16</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>18</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>20</td>
                                <td>-</td>
                                <td>3,90,000</td>
                                <td>32,50,000</td>
                            </tr>



                        </table>
                    </div>
                </div>
                
            </div>
            <div class="row dv-center">
                <div class="col-md-6 ">
                    <div class="rec_table " style="overflow-x:auto; ">
                        <table>
                            <tr>
                                <th colspan="4" class="bg_green_head">KISSAN SAMRIDHI YOJANA (A)</th>
                            </tr>

                            <tr>
                                <th class="subhead">Year</th>
                                <th class="subhead">Contribution
                                    Amt. YEARLY
                                    (10000*12)</th>
                                <th class="subhead">Pay-out After
                                    6th Year
                                    (10000*12)</th>
                                <th class="subhead">Surrender Benefit</th>

                            </tr>

                            <tr>
                                <td>1</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>2</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>3</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>4</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>5</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>6</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>7</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>8</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>10</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>12</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>14</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>16</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>18</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>20</td>
                                <td>-</td>
                                <td>1,20,000</td>
                                <td>10,00,000</td>
                            </tr>



                        </table>
                    </div>
                    <p class="condition pb-30">*SCHEME ONLY FOR MEMBERS</p>
                </div>
                <div class="col-md-6 ">
                    <div class="rec_table " style="overflow-x:auto; margin-top: -50px;"  >
                        <table>
                            <tr>
                                <th colspan="4" class="bg_green_head">KISSAN SAMRIDHI YOJANA (B)</th>
                            </tr>

                            <tr>
                                <th class="subhead">Year</th>
                                <th class="subhead">Contribution
                                    Amt. YEARLY
                                    (25000*12)</th>
                                <th class="subhead">Pay-out After
                                    7th Year
                                    (13000*12)</th>
                                <th class="subhead">Surrender Benefit</th>

                            </tr>

                            <tr>
                                <td>1</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>2</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>3</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>4</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>5</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>6</td>
                                <td>1,20,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>7</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>8</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>10</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>12</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>14</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>16</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>18</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>20</td>
                                <td>-</td>
                                <td>1,56,000</td>
                                <td>13,00,000</td>
                            </tr>



                        </table>
                    </div>
                </div>
            </div>
            <div class="row dv-center">
                <div class="col-md-6 ">
                    <div class="rec_table " style="overflow-x:auto;">
                        <table>
                            <tr>
                                <th colspan="4" class="bg_green_head">KISSAN SAMRIDHI YOJANA (A)</th>
                            </tr>

                            <tr>
                                <th class="subhead">Year</th>
                                <th class="subhead">Contribution
                                    Amt. YEARLY
                                    (2000*12)</th>
                                <th class="subhead">Pay-out After
                                    6th Year
                                    (2000*12)</th>
                                <th class="subhead">Surrender Benefit</th>

                            </tr>

                            <tr>
                                <td>1</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>2</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>3</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>4</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>5</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>6</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>7</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>8</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>10</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>12</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>14</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>16</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>18</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>20</td>
                                <td>-</td>
                                <td>24,000</td>
                                <td>2,00,000</td>
                            </tr>



                        </table>
                    </div>
                    <p class="condition pb-30">*SCHEME ONLY FOR MEMBERS</p>
                </div>
                
                <div class="col-md-6 ">
                    <div class="rec_table " style="overflow-x:auto; margin-top: -50px;"  >
                        <table>
                            <tr>
                                <th colspan="4" class="bg_green_head">KISSAN SAMRIDHI YOJANA (B)</th>
                            </tr>

                            <tr>
                                <th class="subhead">Year</th>
                                <th class="subhead">Contribution
                                    Amt. YEARLY
                                    (2000*12)</th>
                                <th class="subhead">Pay-out After
                                    7th Year
                                    (2600*12)</th>
                                <th class="subhead">Surrender Benefit</th>

                            </tr>

                            <tr>
                                <td>1</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>2</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>3</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>4</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>5</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>6</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>7</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>8</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>10</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>12</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>14</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>16</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>18</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>20</td>
                                <td>-</td>
                                <td>31,200</td>
                                <td>2,60,000</td>
                            </tr>



                        </table>
                    </div>
                </div>
            </div>
            <div class="row dv-center">
                <div class="col-md-6 ">
                    <div class="rec_table " style="overflow-x:auto;">
                        <table>
                            <tr>
                                <th colspan="4" class="bg_green_head">KISSAN SAMRIDHI YOJANA (A)</th>
                            </tr>

                            <tr>
                                <th class="subhead">Year</th>
                                <th class="subhead">Contribution
                                    Amt. YEARLY
                                    (1000*12)</th>
                                <th class="subhead">Pay-out After
                                    6th Year
                                    (1000*12)</th>
                                <th class="subhead">Surrender Benefit</th>

                            </tr>

                            <tr>
                                <td>1</td>
                                <td>12,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>2</td>
                                <td>12,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>3</td>
                                <td>12,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>4</td>
                                <td>12,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>5</td>
                                <td>12,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>6</td>
                                <td>12,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>7</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>8</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>10</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>12</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>14</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>16</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>18</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>20</td>
                                <td>-</td>
                                <td>12,000</td>
                                <td>1,00,000</td>
                            </tr>



                        </table>
                    </div>
                    <p class="condition pb-30">*SCHEME ONLY FOR MEMBERS</p>
                </div>
                <div class="col-md-6 ">
                    <div class="rec_table " style="overflow-x:auto; margin-top: -50px;"  >
                        <table>
                            <tr>
                                <th colspan="4" class="bg_green_head">KISSAN SAMRIDHI YOJANA (B)</th>
                            </tr>

                            <tr>
                                <th class="subhead">Year</th>
                                <th class="subhead">Contribution
                                    Amt. YEARLY
                                    (1000*12)</th>
                                <th class="subhead">Pay-out After
                                    7th Year
                                    (1300*12)</th>
                                <th class="subhead">Surrender Benefit</th>

                            </tr>

                            <tr>
                                <td>1</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>2</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>3</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>4</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>5</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>6</td>
                                <td>24,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>7</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>8</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>10</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>12</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>14</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>16</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>18</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>20</td>
                                <td>-</td>
                                <td>15,600</td>
                                <td>1,30,000</td>
                            </tr>



                        </table>
                    </div>
                </div>
            </div>
            <div class="row dv-center">
                <div class="col-md-6 ">
                    <div class="rec_table " style="overflow-x:auto;">
                        <table>
                            <tr>
                                <th colspan="4" class="bg_green_head">KISSAN SAMRIDHI YOJANA (A)</th>
                            </tr>

                            <tr>
                                <th class="subhead">Year</th>
                                <th class="subhead">Contribution
                                    Amt. YEARLY
                                    (5000*12)</th>
                                <th class="subhead">Pay-out After
                                    6th Year
                                    (5000*12)</th>
                                <th class="subhead">Surrender Benefit</th>

                            </tr>

                            <tr>
                                <td>1</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>2</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>3</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>4</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>5</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>6</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>7</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>8</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>10</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>12</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>14</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>16</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>18</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>20</td>
                                <td>-</td>
                                <td>60,000</td>
                                <td>5,00,000</td>
                            </tr>



                        </table>
                    </div>
                    <p class="condition pb-30">*SCHEME ONLY FOR MEMBERS</p>
                </div>
                <div class="col-md-6 ">
                    <div class="rec_table " style="overflow-x:auto; margin-top: -50px;"  >
                        <table>
                            <tr>
                                <th colspan="4" class="bg_green_head">KISSAN SAMRIDHI YOJANA (B)</th>
                            </tr>

                            <tr>
                                <th class="subhead">Year</th>
                                <th class="subhead">Contribution
                                    Amt. YEARLY
                                    (5000*12)</th>
                                <th class="subhead">Pay-out After
                                    7th Year
                                    (6500*12)</th>
                                <th class="subhead">Surrender Benefit</th>

                            </tr>

                            <tr>
                                <td>1</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>2</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>3</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>4</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>5</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>
                            <tr class="green_row">
                                <td>6</td>
                                <td>60,000</td>
                                <td>-</td>
                                <td>-</td>
                            </tr>


                            <tr>
                                <td>7</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>8</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr>
                                <td>9</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>10</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr>
                                <td>11</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>12</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr>
                                <td>13</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>14</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr>
                                <td>15</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>16</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr>
                                <td>17</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>18</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr>
                                <td>19</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>
                            <tr class="green_row">
                                <td>20</td>
                                <td>-</td>
                                <td>78,000</td>
                                <td>6,50,000</td>
                            </tr>



                        </table>
                    </div>
                </div>



            </div>
            <div class="terms-pro">
                <h6><i> Terms & Conditions of “NATURAL SAMRIDHI YOJANA (NSY)</i></h6>
                <ul>
                    <li>Minimum contribution of Rs.1000/- per month.</li>
                    <li>Contribution to be done for 72 months (6 years) for KSY-A & 84months (7 years) for KSY-B.</li>
                    <li>Pay-out will start from 7th year for KSY-A & 8th year for KSY-B.</li>
                    <li>No prematurity is allowed in KSY.</li>
                    <li>Discontinued accounts will get return after the end of paying term (7/8 years) with savings Interest rates.</li>
                    <li>Contribution instalments can be made monthly. Quarterly, Half Yearly & Yearly, & the pay-out instalments will be as
                        same as the contribution instalment.</li>
                </ul>

            </div>


        </div>

    </div>



    <!--===================================== TESTIMONIAL START ==================================-->
    {{-- @if($testimonials->count() > 0) --}}

    @include('website.testimonial')
    {{-- @endif --}}
    <!--===================================== TESTIMONIAL END ==================================-->
    <!--===================================== MEMBERSHIP START ======================================-->
    <!-- <section class="membership-section" id="memberForm">
    <div class="membership-left">
        <img src="{{ asset('website/images/card.png') }}" alt="Icon">
        <div class="membership-text">
            <h2>Membership</h2>
            <p>
                Membership is an affiliation granted to members who are well involved in agricultural activities within a particular society or community. The membership recognizes one’s significant commitment, dedication, and contribution to the agricultural society.
            </p>
        </div>
    </div>
    <div class="membership-right">
        <h4>Get in Touch</h4>
        <h2>Contact with us</h2>
        <form action="{{ route('website.contact.submit') }}" method="POST" class="contact-form">
            @csrf
            <input type="text" name="name" placeholder="Your Name" value="{{ old('name') }}" required />
            @error('name')
            <div class="text-danger">{{ $message }}</div>
            @enderror

            <input type="email" name="email" placeholder="Email Address" value="{{ old('email') }}" required />
            @error('email')
            <div class="text-danger">{{ $message }}</div>
            @enderror

            <input type="tel" name="phone" placeholder="Phone" value="{{ old('phone') }}" />
            @error('phone')
            <div class="text-danger">{{ $message }}</div>
            @enderror

            <textarea name="message" placeholder="Message">{{ old('message') }}</textarea>
            @error('message')
            <div class="text-danger">{{ $message }}</div>
            @enderror

            <button type="submit">Submit</button>
        </form>
    </div>
</section> -->



    <section class="membership-section" id="memberForm">
        <div class="container">
            <div class="row dv-center">
                <div class="col-sm-4 col-md-4">
                    <div class="membership-left-img" data-aos="fade-right"
                        data-aos-offset="300"
                        data-aos-easing="ease-in-sine">
                        <img src="{{ asset('website/images/card.png') }}" alt="Icon">
                    </div>
                </div>
                <div class="col-sm-4 col-md-4" data-aos="fade-up">
                    <div class="membership-text">
                        <h2>Membership</h2>
                        <p>
                            Membership is an affiliation granted to members who are well involved in agricultural activities within a particular society or community. The membership recognizes one’s significant commitment, dedication, and contribution to the agricultural society.
                        </p>
                    </div>
                </div>
                <div class="col-sm-4 col-md-4 mob-100">
                    <div class="membership-right">
                        <h4>Get in Touch</h4>
                        <h2>Contact with us</h2>
                        <form action="{{ route('website.contact.submit') }}" method="POST" class="contact-form">
                            @csrf
                            <input type="text" name="name" placeholder="Your Name" value="{{ old('name') }}" required />
                            @error('name')
                            <div class="text-danger">{{ $message }}</div>
                            @enderror

                            <input type="email" name="email" placeholder="Email Address" value="{{ old('email') }}" required />
                            @error('email')
                            <div class="text-danger">{{ $message }}</div>
                            @enderror

                            <input type="tel" name="phone" placeholder="Phone" value="{{ old('phone') }}" />
                            @error('phone')
                            <div class="text-danger">{{ $message }}</div>
                            @enderror

                            <textarea name="message" placeholder="Message">{{ old('message') }}</textarea>
                            @error('message')
                            <div class="text-danger">{{ $message }}</div>
                            @enderror

                            <button type="submit">Submit</button>
                        </form>
                    </div>
                </div>
            </div>

        </div>
    </section>
    <!--===================================== MEMBERSHIP END ======================================-->
</div>

@endsection